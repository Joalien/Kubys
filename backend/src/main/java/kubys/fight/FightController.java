...
public class FightController {
    ...
    public class FightUuidController {

        @MessageMapping("/move/{cellId}")
        @SendTo("/broker/fight/{fightUuid}")
        public void moveToCell(SimpMessageHeaderAccessor headerAccessor, @DestinationVariable int cellId) {
            log.info("moveToCell " + cellId);
        }

        @MessageMapping("/use/{spellId}/on/{cellId}")
        @SendTo("/broker/fight/{fightUuid}")
        public void useSpellOnCell(SimpMessageHeaderAccessor headerAccessor, @DestinationVariable int spellId, @DestinationVariable int cellId) {
            log.info("use " + spellId + " on " + cellId);
        }
        ...
        public boolean winFight(SimpMessageHeaderAccessor headerAccessor, @DestinationVariable String fightUuid) {
            log.info("winFight : " + fightUuid + " uuid");
